...
const hexCodeToFilter = (hexCode) => {
    const rgb = hexToRgb(standardize_color(hexCode));
    if (typeof rgb == "undefined" || rgb == null || rgb.length !== 3) return '';
...
mfobj.parse  = function(text, _settings = {}) { 
  if (mfobj.isMobile() == true) {
    ...
    return text.split('￼').join(`<img src="https://cdn.jsdelivr.net/gh/bribes/mobile-obj-parser/images/obj_black.png" style="${hexCodeToFilter(settings.color)}" width="${settings.width}" height="${settings.height}" class="mobile-obj">`);
  } else {
    return text;
  }
}
